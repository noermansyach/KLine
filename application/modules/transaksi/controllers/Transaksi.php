...
class Transaksi extends MX_Controller 
{
	...
	public function index() 
	{
		...
		$userAkses = $this->session->userdata('position');
		if ($userAkses == '10') {
			$this->_data['js'] = "layout-part/index-js";
			$this->_data['content'] = "index_admin";
		} elseif ($userAkses == '9') {
			$this->_data['js'] = "layout-part/index-manager-js";
			$this->_data['content'] = "index_manager";
		} elseif ($userAkses == '4'){
			$this->_data['content'] = "index_direktur";
		}
		

		$table = $this->transaksi->readAllOperational();
	...
	public function add() 
	{
		...
		$this->_data['principalList'] = $this->transaksi->getPrincipal();
		$this->_data['departementList'] = $this->transaksi->getDepartment($this->_company);
		$this->_data['tanggalBuat'] = date('d-m-Y');
		$this->_data['vesel'] = $this->transaksi->getVesel();
		// $this->_data['acknowledgeList'] = $this->transaksi->getAcknowledgeList();
		// $this->_data['approvedList'] = $this->transaksi->getApprovedList();
		$this->_data['accBankData'] = $this->transaksi->getAccBankPerkiraan();

		if ($_POST) {
			$vesel = "-";
			if ($_POST['selectVesel'] != '') {
				$vesel = $_POST['selectVesel'];
			}

			$dataPPU = array(
				'no_transaksi' => $_POST['noBukti'],
				'no_ppu' => $_POST['noPPU'],
				...
				'keterangan_satu' => $_POST['ketSatu'], 
				'keterangan_dua' => $_POST['ketDua'],
				'keterangan_tiga' => $_POST['ketTiga'],
				'qr_code' => "",
				'report' => "",
				'attachment' => "",
				'id_kunjungan' => $vesel,
				'tipe_transaksi' => $_POST['receivePayment'],
				'created_time' => date('Y-m-d H:i:s'),
				'created_by' => $this->session->userdata('employeeId'),
				'updated_time' => date('Y-m-d H:i:s'),
				'updated_by' => $this->session->userdata('employeeId')
			);
			print_r($this->transaksi->insertTransaksi($dataPPU));
	...
	public function approveManager($noTransaksi) {
		$dataApproveMGR = array(
			'acknowledge_by' => $this->session->userdata('employeeId'),
		);
		$this->transaksi->updateTransaksi($dataApproveMGR, $noTransaksi);
		redirect('transaksi', 'refresh');
	}

	public function getDataPPU($noTransaksi) {
		$dataPPU = $this->transaksi->getDataPPUdua($noTransaksi);
		echo json_encode($dataPPU);
	}
